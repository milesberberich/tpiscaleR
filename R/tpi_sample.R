...
smart_sample <- function(r, n) {
  valid_indices <- which(!is.na(values(r, dataframe=FALSE)))
  selected_indices <- sample(valid_indices, size = n)
  pts <- vect(xyFromCell(r, selected_indices), crs = crs(r))
  values(pts) <- extract(r, pts)
  return(pts)}
...
tpi_sample <- function(raster_dsm, raster_target, sample_num, radius, relationship = "spearman") {
  ...
  raster_target_masked <- mask(raster_target, raster_dsm) # to prevent to have no elevation information in the target_raster, we mask here

  random_points <- smart_sample(raster_target_masked, sample_num)

  ele <- extract(dsm, random_points)# extracting elevation into the ranodom
  random_points_ele <- random_points
  random_points_ele$elevation <- ele[, 2]

  random_points_buffer <- buffer(random_points_ele, radius)
  # This is functionally the same as focal() but only calculated at these spots
  random_points_meanele <- random_points_buffer
  random_points_meanele$mean_elevation <- extract(dsm, random_points_buffer, fun = "mean",
